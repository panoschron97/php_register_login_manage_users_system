...
$userthatisconnected = $_SESSION["username"];

$result = mysqli_query($connection, "SELECT id, username, email FROM users WHERE NOT username = '$userthatisconnected'");
...
$new_email = strtolower($new_email);
        ...
        $query_status = check_query($connection, updater_user($connection, $id, $new_username, $new_email));
...
elseif(isset($_POST['delete_user']))
    {
        $id = mysqli_real_escape_string($connection, $_POST['id']);
        $query_status = check_query($connection, delete_user($connection, $id));
        if($query_status == true){
            $_SESSION['message'] = "User deleted successfully with id : {$id}!";
            $_SESSION['msg_type'] = "success";
            sleep(5);
            redirect("admin.php");
        }
